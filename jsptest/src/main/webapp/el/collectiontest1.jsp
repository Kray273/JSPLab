...
<!-- arraylist와 list 차이 동적,정적  형변환하지않아도 o-->
${list.get(0).id } : ${ list[0] } <br> <!-- 아무것도 안쓰면 .toString 출력됨 -->
...
%>
<!-- 저장했던 순서와 일치하지 않음, 인덱스 사용불가x -->


${ colormap["빨강"] }<br> <!-- 한글은 이렇게 사용 -->
${ empty colormap["빨강색"] ? "해당색상없음": colormap["빨강색"] }<br>
${ colormap.one } <!-- 포함이 되어있는것처럼 사용가능 --><br>
<!-- 키중복 불가능해서 새로운값 넣으면 마지막값만 사용가능 -->
